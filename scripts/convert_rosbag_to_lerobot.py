...
class AllIdrVideoBuffer:
    ...
    def _mux_to_ts(self, packet_list, info) -> str:
        ...
        # final tick can go unemitted when the last real frame lands just
        # before it. The sentinel's own (re-decoded) picture is never output —
        # it lies beyond the -frames:v cap.
        sentinel = av.Packet(packet_list[-1]['data'])
